...
const { viewEmployees, viewEmployeesByManager, addEmployee, addEmployeePrompt , deleteEmployeePrompt,deleteEmployee,employeeUpdate,employeeUpdatePrompt} = require('./employee');
const { viewDepartments , addDepartment,addDepartmentPrompt,deleteDepartment,deleteDepartmentPrompt} = require('./deparment');
const { viewRoles, addRole, addRolePrompt, deleteRole,deleteRolePrompt, viewTotalSpendingBudget } = require('./role');   //imports for all the functions that are used in the menu options
...
const startMenuPrompt = [ {  //this is the prompt for the initial menu
    name:'selection',
    type:'list',
...
startMenu = () => {     //this function handles serving the inital options when the appliction starts
   return  inquirer.prompt( startMenuPrompt
    ...
    answers.selection ==='Delete Role' ? deleteOperation(deleteRolePrompt,deleteRole) : null;
    answers.selection ==='Delete Employee' ? deleteOperation(deleteEmployeePrompt,deleteEmployee) : null;
    answers.selection ==='Update Employees Role'? updateOperation(employeeUpdatePrompt,employeeUpdate) : null;
    answers.selection ==='View Salary Budget' ? viewOperation(viewTotalSpendingBudget) : null;
    answers.selection ==='Exit' ? true : null;
      
   })
}

function printTableWithoutIndex(data, chalk) {  // this function takes in the data from the database and chalk and prints it to the console
...
// goes over each row and updates them with the columns max width for each item
  ...
  const deleteOperation = (questions, deleteSQL) => { // this is a delete operation that will take in questions and sql lines
    return inquirer.prompt(questions)
      .then((response) => {
        return deleteSQL(response);
      })
      .then(() => startMenu());
  }

  const updateOperation = (questions, updateSQL) => { // this is an update operation that will take in questions and sql lines
    return inquirer.prompt(questions)
           .then((response) => {
            return updateSQL(response);
           })
           .then(() => startMenu());
  }
